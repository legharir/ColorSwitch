function TriangleObstacle(x, y) {
	...
	this.show = function() {
		// Getting the this.coordinates of the triangle vertithis.ces
		var left = [this.x - this.radius * Math.cos(PI / 6), this.y - this.radius * Math.sin(PI / 6)];
		var right = [this.x + this.radius * Math.cos(PI / 6), this.y - this.radius * Math.sin(PI / 6)];
		var top = [this.x, this.y + this.radius];

		// Drawing the bottom line
		stroke(colours[this.c % 3][1], colours[this.c % 3][2], colours[this.c % 3][3]);
